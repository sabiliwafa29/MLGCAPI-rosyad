...
const serviceAccount = require('/home/MLGCAPI/submissionmlgc-rosyad-ee74f442da43.json'); 
...
app.post('/predict', (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      await storeData(predictionResult.data.id, predictionResult.data);
      res.send({
        status: 'success',
...
app.get('/predict/histories', async (req, res) => { 
  try { 
    ...
    res.send({ 
      ...
      res.status(500).send({ 
        status: 'fail', 
        message: `Terjadi kesalahan dalam mendapatkan histori ${error.message}` 
      }); 
    } 
});
